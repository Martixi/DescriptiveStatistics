...
```
# Let's go on and vizualise the survival rates in terms of age
```{r}
ggplot(cleanStatus, aes(x = Status, y = Age, fill = Status)) +
  geom_boxplot() +
  labs(title = "Age Distribution by Survival Status",
       x = "Survival Status",
       y = "Age") +
  scale_fill_brewer(palette = "Set1", labels = c("Victims", "Survivors")) +
  theme_minimal()

ggplot(cleanStatus, aes(x = Age, fill = Status, color = Status)) +
  geom_density(alpha = 0.5) +
  labs(title = "Density of Age by Survival Status",
       x = "Age",
       y = "Density") +
  scale_fill_brewer(palette = "Set1", labels = c("Victims", "Survivors")) +
